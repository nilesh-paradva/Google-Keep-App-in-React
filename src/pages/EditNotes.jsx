...
import { SingleNotesThunk, UpdateNotesThunk } from "../services/action/GoogleKeepAction";

const EditNotes = () => {

    const { note, isCreated, loading } = useSelector(state => state.GoogleKeepReducer);
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        dispatch(UpdateNotesThunk(addNotes))
    }

    useEffect(() => {
        dispatch(SingleNotesThunk(id))
    }, [id])
                    ...
                    <Row>
                        <Col lg={12}>
                            <div className="max-w-lg mx-auto px-4 py-6 bg-[#4B5945] rounded-lg shadow-lg">
                                <h1 className="text-2xl font-bold text-center mb-6 text-[#B2C9AD]">Edit Notes</h1>
                                <form onSubmit={handleSubmit} className="space-y-4">
                                    ...
                                    <div className="notes-submit-button text-center">
                                        <button type="submit" className=" bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-md shadow-md"> {loading ? "Updating..." : "Update Note"} </button>
                                    </div>
                                </form>
                            </div>
                        </Col>
